...
namespace X.Core.Utility
{
    ...
    public class Tools
    {
        ...
        private static HttpContext c = HttpContext.Current;
        #endregion
        ...
        public static string SaveImage(string base64)
        {
            ...
            var path = "/upload/" + DateTime.Now.ToString("yyyy-MM-dd") + "/";
            var di = new DirectoryInfo(c.Server.MapPath(path));
            if (!di.Exists) di.Create();

            var fname = Secret.MD5(base64);
            fname = fname + getpix(img.RawFormat);
            img.Save(di + fname, img.RawFormat);
        ...
        static string getpix(ImageFormat fr)
        {
            if (fr == ImageFormat.Jpeg) return ".jpg";
            if (fr == ImageFormat.Gif) return ".gif";
            if (fr == ImageFormat.Png) return ".png";
            return ".jpg";
        }
        #endregion
