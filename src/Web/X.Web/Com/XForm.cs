...
namespace X.Web.Com
{
    public class XForm
    {
        ...
        class text : ct, ict
        {
            ...
            {
                if (tp == 1)
                    return "<input type='text' id='id_" + name + "' name='" + name + "' x-check='" + c.ToString() + "' title='" + (string.IsNullOrEmpty(tip) ? title : tip) + "' placeholder='" + (string.IsNullOrEmpty(tip) ? title : tip) + "' class='input-" + sizes[size - 1] + "' value='" + def + "' />";
                else if (tp == 2)
                    return "<input type='password' id='id_" + name + "' name='" + name + "' title='" + (string.IsNullOrEmpty(tip) ? title : tip) + "' x-check='" + c.ToString() + "' placeholder='" + (string.IsNullOrEmpty(tip) ? title : tip) + "' class='input-" + sizes[size - 1] + "' value='" + def + "' />";
                else if (tp == 3)
                    return string.Format("<textarea id='id_{0}' name='{0}' placeholder='{1}' x-check='{2}' class='input-{3}' title='" + (string.IsNullOrEmpty(tip) ? title : tip) + "' >{4}</textarea>",
                             name,
            ...
            public void init(string body)
            {
                ...
                if (mode == 2 && (data[0] == "dict" || data[0] == "loc"))
                {
                    ...
                    if (data[0] == "dict")
                    {
                        ...
                        if (d != null) foreach (var i in d) sb_html.Append("<span class='btn " + (def == i.value || (def != null && def.Contains("[" + i.value + "]")) ? "btn-primary" : "") + "' name='" + name + "' x-val='" + i.value + "'>" + i.name + "</span>");
                    }
                    else
                    {
