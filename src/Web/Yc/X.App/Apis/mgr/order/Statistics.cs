...
namespace X.App.Apis.mgr.order {
    public class Statistics{
        ...
        public static int[] getOrderStati(DateTime startTime, int size,DataClassesDataContext DB) {//应该可以改成table直接处理
            ...
            var dayList = from q in DB.x_order where q.ctime > startTime select q;//取出当天订单
        ...
        public static int[] getMemberStati(DateTime startTime, int size, DataClassesDataContext DB) {//应该可以改成table直接处理
            //startTime-开始查询的时间,此处需要已经处理好为当天时间（0:00）
            //size-实际可返回的数最大为24
            ...
            var dayList = from q in DB.x_user where q.ctime > startTime select q;//取出当天订单

            var groupList = dayList.GroupBy(o => o.ctime.Hour)
                            .Select(g => new {
                                hour = g.Key,
                                count = dayList.Where(o => o.ctime.Hour == g.Key).Count()
                            }).ToArray();
