...
namespace X.App.Apis.mgr.refund
{
    public class accept : xmg
    {
        ...
        protected override XResp Execute()
        {
            ...
            if (od == null) throw new XExcep("T退款记录不存在");
            if (od.status != 1) throw new XExcep("T当前退款已经处理");
            ...
            if (od.x_order.pay_way == 1)
            {
                ...
                if (response.return_code.Equals("FAIL")) throw new XExcep("T微信退款失败", response.return_msg + "，" + response.err_code_des);
            }
