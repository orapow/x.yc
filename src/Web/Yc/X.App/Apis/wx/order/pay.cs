...
namespace X.App.Apis.wx.order
{
    public class pay : _wx
    {
        ...
        private XResp wechatPay()
        {
            ...
            if (!string.IsNullOrEmpty(c)) throw new XExcep("T当前已经有操作正在进行");

            var od = cu.x_order.FirstOrDefault(o => o.order_id == id);
            if (od.iscancel==true) throw new XExcep("T订单已经取消");
            if (od.pay_amount > 0) throw new XExcep("T当前订单已经支付");
            if (od.status != 1) throw new XExcep("T订单当前状态不能支付");
            ...
            if (co.return_code == "FAIL") throw new XExcep("T微信支付失败", co.return_msg);
            if (co.result_code == "FAIL") throw new XExcep("T微信支付失败", co.err_code + "," + co.err_code_des);
            if (string.IsNullOrEmpty(co.prepay_id)) throw new XExcep("T微信支付预付ID为空");
